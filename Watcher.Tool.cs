    ...
    */
    ...
    public bool ItemOfPlayerCheck(TSPlayer? player, out string mess)
    {
        ...
        if (player == null)
        {
            foreach (TSPlayer v in TShock.Players)
            {
                ...
                if (!ReasonableItem(items, out Dictionary<Item, string> abItem1) | !ReasonableItemOfRecipe(items, out Dictionary<Item, string> abItem2))
                {
                    ...
                    temp.AddRange(abItem1.Values);
                    temp.AddRange(abItem2.Values);
                    mess += Warning(v, TypesOfCheat.ItemCheat, (TypesOfPunish)config.物品作弊警告方式, config.物品作弊是否算违规, "", 0, 0, Vector2.Zero, temp) + '\n';
                    if (config.检测到违禁物时是否清空 && v != null && v.IsLoggedIn)
                    {
                        List<Item> list = new List<Item>();
                        list.AddRange(abItem1.Keys);
                        list.AddRange(abItem2.Keys);
                        ClearPlayersItem(list, v);
                        if (config.物品作弊警告方式 != 0)
        ...
        else if (NeedBeChecked(player))
        {
            ...
            if (!ReasonableItem(items, out Dictionary<Item, string> abItem1) | !ReasonableItemOfRecipe(items, out Dictionary<Item, string> abItem2))
            {
                ...
                mess += Warning(player, TypesOfCheat.ItemCheat, (TypesOfPunish)config.物品作弊警告方式, config.物品作弊是否算违规, "", 0, 0, Vector2.Zero, temp);
                flag = true;
    ...
    public bool ReasonableItem(List<Item> items, out Dictionary<Item, string> abItem)
    {
        ...
        foreach (Item i in items)
        {
            ...
            {
                var temp = abItem.Where(x => x.Key.netID == 1309 || x.Key.netID == 905);
            ...
            if (i.type == 86)
            {
                for (int s = 0; s < 200; s++)
                {
                    ...
                    {
                        var temp = abItem.Where(x => x.Key.netID == 86);
                        foreach (var v in temp)
                        {
                            abItem.Remove(v.Key);
                        }
    ...
    public void ClearPlayersItem(ICollection<Item> items, TSPlayer tSPlayer)
    {
        if (tSPlayer == null || !tSPlayer.IsLoggedIn)
        {
            return;
        }
        var itemIDs = items.Select(x => x.type);
        Utils.ClearPlayerItems(itemIDs, tSPlayer.TPlayer.inventory, PlayerItemSlotID.Inventory0, tSPlayer.Index);
        Utils.ClearPlayerItems(itemIDs, tSPlayer.TPlayer.armor, PlayerItemSlotID.Armor0, tSPlayer.Index);
        Utils.ClearPlayerItems(itemIDs, tSPlayer.TPlayer.Loadouts[0].Armor, PlayerItemSlotID.Loadout1_Armor_0, tSPlayer.Index);
        Utils.ClearPlayerItems(itemIDs, tSPlayer.TPlayer.Loadouts[1].Armor, PlayerItemSlotID.Loadout2_Armor_0, tSPlayer.Index);
        Utils.ClearPlayerItems(itemIDs, tSPlayer.TPlayer.Loadouts[2].Armor, PlayerItemSlotID.Loadout3_Armor_0, tSPlayer.Index);
        Utils.ClearPlayerItems(itemIDs, tSPlayer.TPlayer.dye, PlayerItemSlotID.Dye0, tSPlayer.Index);
        Utils.ClearPlayerItems(itemIDs, tSPlayer.TPlayer.Loadouts[0].Dye, PlayerItemSlotID.Loadout1_Dye_0, tSPlayer.Index);
        Utils.ClearPlayerItems(itemIDs, tSPlayer.TPlayer.Loadouts[1].Dye, PlayerItemSlotID.Loadout2_Dye_0, tSPlayer.Index);
        Utils.ClearPlayerItems(itemIDs, tSPlayer.TPlayer.Loadouts[2].Dye, PlayerItemSlotID.Loadout3_Dye_0, tSPlayer.Index);
        Utils.ClearPlayerItems(itemIDs, tSPlayer.TPlayer.miscEquips, PlayerItemSlotID.Misc0, tSPlayer.Index);
        Utils.ClearPlayerItems(itemIDs, tSPlayer.TPlayer.miscDyes, PlayerItemSlotID.MiscDye0, tSPlayer.Index);
        Utils.ClearPlayerItem(itemIDs, tSPlayer.TPlayer.trashItem, PlayerItemSlotID.TrashItem, tSPlayer.Index);
        Utils.ClearPlayerItem(itemIDs, tSPlayer.TPlayer.inventory[tSPlayer.TPlayer.selectedItem], PlayerItemSlotID.InventoryMouseItem, tSPlayer.Index);
        Utils.ClearPlayerItems(itemIDs, tSPlayer.TPlayer.bank.item, PlayerItemSlotID.Bank1_0, tSPlayer.Index);
        Utils.ClearPlayerItems(itemIDs, tSPlayer.TPlayer.bank2.item, PlayerItemSlotID.Bank2_0, tSPlayer.Index);
        Utils.ClearPlayerItems(itemIDs, tSPlayer.TPlayer.bank3.item, PlayerItemSlotID.Bank3_0, tSPlayer.Index);
        Utils.ClearPlayerItems(itemIDs, tSPlayer.TPlayer.bank4.item, PlayerItemSlotID.Bank4_0, tSPlayer.Index);
        for (int i = 0; i < 22; i++)
        {
