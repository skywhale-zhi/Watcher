...
public partial class Watcher : TerrariaPlugin
{
    ...
    private void OnChat(ServerChatEventArgs args)
    {
        ...
        if (tSPlayer == null || !tSPlayer.Active || !config.是否把对话内容写入日志)
        {
            return;
        }
    ...
    private void OnNewProjectile(object? sender, GetDataHandlers.NewProjectileEventArgs e)
    {
        ...
        if (config.危险射弹广播警告 && CheatData.DangerousProjectile.Contains(e.Type))
        {
            ...
            if (wp != null)
            {
                ...
                {
                    Warning(e.Player, TypesOfCheat.DangerousProj, (TypesOfPunish)config.生成危险射弹警告方式, false, Lang.GetProjectileName(e.Type).Value, 0, e.Type, e.Position);
                }
            }
        }
        ...
        if (config.启用伤害检测 && NeedBeChecked(e.Player))
        {
            ...
            if (e.Damage > config.射弹最大伤害 && !isExclude)
            {
                ...
                {
                    wp.伤害检测位++;
                    Warning(e.Player, TypesOfCheat.DamageCheat, (TypesOfPunish)config.伤害作弊警告方式, config.伤害作弊是否算违规, Lang.GetProjectileName(e.Type).Value, e.Damage, e.Type, e.Position);
                }
            }
        }
        #endregion
        ...
        if (config.启用浮标数目检测 && NeedBeChecked(e.Player))
        {
            ...
            if (target != 0)
            {
                Warning(e.Player, TypesOfCheat.FishCheat, (TypesOfPunish)config.钓鱼作弊警告方式, config.钓鱼作弊是否算违规, Lang.GetProjectileName(target).Value, 0, target, e.Position);
            }
        }
        #endregion
    }
    ...
    private void OnNpcStrike(NpcStrikeEventArgs args)
    {
        ...
        if (args.Damage > config.其他最大伤害 && !isExclude)
        {
            ...
            {
                wp.伤害检测位++;
                Warning(TShock.Players[args.Player.whoAmI], TypesOfCheat.DamageCheat, (TypesOfPunish)config.伤害作弊警告方式, config.伤害作弊是否算违规, "", args.Damage, 0, args.Player.position);
            }
        }
    }
